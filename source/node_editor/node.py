...
from source.node_editor.gui.node_graphics import Node_Graphics
...
class Node(Node_Graphics):
    ...
    def delete(self):
        ...
        for connection in to_delete:
            #for conn in connection: #index 1 is deleted
            #now index2 becomes index 1
    ...
    def select_connections(self, value):
        ...
        for pin in self._pins:
            if pin.connection:
                for conn in pin.connection:
                    if conn:
                        conn._do_highlight = value
                        conn.update_path()
