from source.node_editor.gui.pin_graphics import Pin_Graphics
...
class Pin(Pin_Graphics):
    ...
    def clear_connection(self):
        """
        if self.connection:
            self.connection.delete()  # delete the connection
        """
        for conn in self.connection:
            ...
            def delete(self):
                ...
                #how scene is defined here
                #where were connection.scene initialized
            """

    def can_connect_to(self, pin):
        if not pin:
            return False
        if pin.node == self.node: #mynf3sh y3ml connect m3 nafso
            return False
        print("it's okay to connect it")
        return self.is_output != pin.is_output


    def is_connected(self):
        if self.connection:
            return bool(len(self.connection))
        return False
