...
class NodeEditor(QtCore.QObject):
    ...
    def __init__(self, parent):
        """
        Constructor for RoutineEditor.

        :param parent: The parentItem widget.
        :type parent: QWidget
        """
    ...
    def install(self, scene):
        """
        Installs the RoutineEditor into a QGraphicsScene.

        :param scene: The QGraphicsScene to install the RoutineEditor into.
    ...
    def eventFilter(self, watched, event):
        ...
        if event.type() == QtCore.QEvent.GraphicsSceneMouseDoubleClick:
            if event.button() == QtCore.Qt.LeftButton:
                ...
                if isinstance(item, Node):
                    ...
                    node = item  # object has attribute called 'item'
                    self.modify_node.emit(node)
                    self.view.pan = False
                    #print("ww")

                #print("double creaaaaaaam!")
        if event.type() == QtCore.QEvent.GraphicsSceneMousePress:
            if event.button() == QtCore.Qt.LeftButton:
                ...
                if isinstance(item, Pin):
                    ...
                    self.connection.being_connected=True
                    self.scene.addItem(self.connection)  ##<<<<<==============================
                    self.port = item
                    self.port.being_connected=True
                    self.connection.start_pos = item.scenePos()
                    self.connection.end_pos = event.scenePos()
                    #print("Pin connection end position: ", self.connection.end_pos)
                ...
                #####################################################################################
                # update connections when we move a node                              #
                if isinstance(item, Node):  #
        ...
        elif event.type() == QtCore.QEvent.GraphicsSceneMouseRelease:  #if event.type() == QtCore.QEvent.MouseButtonRelease:
            #what if i released the mouse button at connected pin
            if self.connection and event.button() == QtCore.Qt.LeftButton:
                item = self.item_at(event.scenePos())
                if self.port:
                    self.port.being_connected = False
                if self.connection:
                    self.connection.being_connected=False
                # connecting a port
                if isinstance(item, Pin):
                    if self.port.can_connect_to(item):
                        ...
                        # delete existing connection to the original port(pin)
                        #self.port.clear_connection() #self.port is the port of the original connection (pin)
                        #item.clear_connection()
                        #connect connection to the port
        ...
        #return QtCore.QObject().eventFilter(watched, event)
        return super().eventFilter(watched, event)  #return instanceOfThatClass.eventFilter(watched,event)
