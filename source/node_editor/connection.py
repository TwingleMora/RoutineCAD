from source.node_editor.gui.connection_graphics import Connection_Graphics
...
class Connection(Connection_Graphics):
    ...
    def delete(self):
        ...
        for pin in (self.start_pin, self.end_pin):
            if pin:
                pin.connection.remove(self)
                #pin.connection = None
            #pin = None

        self.scene().removeItem(self)

    def set_start_pin(self, pin): #string ex(value, inputA, inputB, Output)
        self.start_pin = pin
        self.start_pin.connection.append(self)
        #self.start_pin.connection = self

    def set_end_pin(self, pin):
        self.end_pin = pin
        self.end_pin.connection.append(self)
    ...
    def update_start_and_end_pos(self):
        """
        Update_Signal the start and end positions of the Connection.

        Get the start and end pins and use them to set the start and end positions.
        """
        ##########################################################################################

        # when connecting from output pin to input pin we need to swap the start and end positions
